...
class CRUDOneTimePassword(
    CRUDBase[OneTimePassword, OneTimePasswordCreate, OneTimePasswordUpdate]
):
    ...
    def get_by_verification_code(
        self, db: Session, *, verification_code: str
    ...
    ) -> List[OneTimePassword]:
        return (
            db.query(OneTimePassword).filter(OneTimePassword.user_id == user_id).all()
        )
    ...
    def get_valid_code(
        ...
        return otp if otp and otp.expires_at > datetime.now(timezone.utc) else None

    def create_apple_review_team_otp(self, db: Session, user) -> OneTimePassword:
        assert (
            user.email == settings.APPLE_REVIEW_TEAM_EMAIL
        ), "User is not the Apple review team user"
    ...
    def get_expired_otps(self, db: Session) -> List[OneTimePassword]:
        return (
            db.query(OneTimePassword)
            .filter(OneTimePassword.expires_at < datetime.now(UTC))
            .all()
        )
